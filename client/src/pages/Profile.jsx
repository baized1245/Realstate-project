...
import { updateUserStart, updateUserSuccess, updateUserFailure, deleteStart, deleteUserSuccess, deleteFailure, signOutUserStart, signOutUserSuccess, signOutUserFailure } from '../redux/user/userSlice';
    ...
    <div className='p-3 max-w-lg mx-auto'>
      ...
      <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
        ...
        <p className='text-center self-center'>
          {
            fileUploadError ? 
            (<span className='text-red-700 text-xl'>Error Image upload (Image must be less then 2 mb)</span>) : 
            filePerc > 0 && filePerc < 100 ? (
              <span className='text-slate-700 text-xl'>{`Uploading ${filePerc}%`}</span>)
              :
              filePerc === 100 ? (
        ...
        <input onChange={handleChange} type="text" placeholder='username' id='username' defaultValue={currentUser.username} className='border p-3 rounded-lg' />
        ...
        <Link className='bg-green-700 text-white p-3 rounded-lg uppercase text-center hover:opacity-80' to={'/create-listing'}>Creat Listing</Link>
      </form>
      ...
      <div className='flex flex-col gap-4'>
            <h1 className='text-center mt-7 text-2xl text-semibold'>Your Listings</h1>
